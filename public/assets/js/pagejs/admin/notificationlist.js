var NotificationListUrl ='/postnotificationlist';
...
var ViewModel=function() {
    ...
    self.NotificationListArray=ko.observableArray();
    ...
    self.PageLoad = function () {
        ...
        AjaxCall(NotificationListUrl, ko.toJSON({ Data: param }), "post", "json", "application/json").done(function (response) {
            if (response.IsSuccess) {
                ko.mapping.fromJS(response.Data.NotificationListArray,'', self.NotificationListArray);
                ...
                if(self.NotificationListArray().length<=0)
                    $('#nodata').show();
                else
                    $('#nodata').hide();
            }
